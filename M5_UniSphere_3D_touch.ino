...
const float TOUCH_SENSITIVITY = 0.01f;  // タッチ操作時の回転の感度

float velocityX = 0.0f;
float velocityY = 0.0f;
const float FRICTION = 0.92f;  // 摩擦係数
...
void loop(void) {
  ...
  if (touch.isPressed()) {
    if (!isDragging) {
      ...
      // ドラッグ中の移動量を計算し、現在の速度にする
      int deltaX = touch.x - prevTouchX;
      int deltaY = touch.y - prevTouchY;

      velocityY = deltaX * TOUCH_SENSITIVITY;
      velocityX = deltaY * TOUCH_SENSITIVITY;
  ...
  if (rotationY < 0) rotationY += TWO_PI;
  if (rotationX > TWO_PI) rotationX -= TWO_PI;
  if (rotationX < 0) rotationX += TWO_PI;
